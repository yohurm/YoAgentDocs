...
status: active
when: modify
description: 审查已有代码的分层与质量。用户说审查架构、硬编码、补丁、越级、MVVM、数据链路、高内聚、单一职责、文件堆职责、API 里写了实现、耦合 internal 时使用。
when_to_use: 架构审查、代码审查、硬编码、补丁层、越级、MVVM、数据链路、单一职责、API 耦合实现
triggers: [架构审查, 代码审查, 硬编码, 补丁, 越级, MVVM, 数据链路, 高内聚, 耦合, 单一职责, 堆在一个文件, API实现, api/]
inputs: [审查范围：模块或用户路径]
outputs: [链路说明, 问题清单（含文件职责与 API 越界）, 若修复则含通路证据]
...
对给定模块或用户路径做分层审查。除数据链路与越级外，**必须**检查：文件是否单一职责、公开 API 是否夹带实现。有问题在正确层修，不加包装。
