...
在正确层修好本轮问题，并用类型包方式验证。不要用无关层的分支把现象盖住。禁止在旧架构上继续打补丁或做防御性加固。同链路暴露的分层问题纳入本轮清零，不得记成「非阻塞后续」或「已知债务」。允许按链路分阶段，但本阶段相关通路必须整理到干净，**严禁残留技术债务**。
...
按手册。严格排查完整数据链路（入口 → 状态持久化/持有 → 业务变换 → 副作用终点）。严禁未经证实的猜测与假设；链路节点不清晰、报错原因或机制拿不准时，必须继续深入链路追踪、结合真实运行时/日志/断点，并联网搜索对应问题的权威文档与解决方案。定位后：

- 局部逻辑错误且现有分层仍正确 → 行为修复。
- 边界/越级/补丁堆积/兼容层，或拟议改动是在旧架构上打补丁/加固 → 升级 [architecture-review.md](architecture-review.md)；实现路径不明则先 [online-deep-research.md](online-deep-research.md)。重设计完整通路并清理旧代码，不要继续修补。
...
- 未扩大到无关模块；同链路架构债与技术债务已清零，无「非阻塞」挂账。
...
- 不把「顺便重构」塞进无关模块；本阶段相关链路上的分层问题必须重构到干净，不算顺便。
- 不在问题未全部达标时写收尾总结、列「后续建议」「已知债务」或「非阻塞项」代替继续修。
- 不在本阶段相关通路留下 TODO/FIXME、跳过测试或兼容双轨当修复完成。
- 不把「已定位 / 已改一处 / 理论上应该好了」当成完成。
- 不基于猜测或假设下结论；拿不准机制时不得闭门推断，必须深入查链路并联网检索方案。
