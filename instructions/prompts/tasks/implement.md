...
把约定能力做完整，通过类型包门禁，而不是只接一个 API。本轮声明要实现的模块或通路必须一次做到干净分层：旧路径删除、无双轨、无「先接上以后再拆」。允许按模块分阶段交付，每一阶段都是可独立成立的干净切片，不是半成品。
...
- 无新增散落硬编码与越级调用；本轮范围内无兼容层、双轨 API 或未删旧路径。
- **本阶段范围内技术债务零残留**（无 TODO/FIXME 替身、无跳过测试、无例外清单扩容、无非阻塞挂账）。
...
- 不把范围内未完成的分层整理写成非阻塞后续或已知债务；下一阶段须另声明范围。
- 不在本阶段范围内留下 TODO/FIXME/HACK、未实现桩或跳过测试当交付。
