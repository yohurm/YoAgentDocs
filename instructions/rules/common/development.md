...
- 注释只解释「为什么」或非显而易见的约束；不复述代码字面意思。严禁写入「某某章节实现」、「第X步/步骤实现」、「根据需求/规范实现」等任务标记、过程描述或元信息，只写入代码自身应有的必要技术注释。
- 删除代码时同时删除真正的死引用；不要留下注释掉的大段旧实现。
- **本阶段范围内严禁残留技术债务：** 不新增、不保留 `TODO` / `FIXME` / `HACK` / `XXX` 当未完成工作的替身；不留未实现桩、跳过测试、双轨开关。做不完就不要标完成，把未完成项留在本轮清单上继续做。范围外既有标记不在本 diff 顺手清，但相关链路上的必须清零。见 [architecture.md](architecture.md)「严禁残留技术债务」。
