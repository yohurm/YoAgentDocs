...
2. 实现手段不明、或现有分层只能靠打补丁撑住，则先走联网深研任务，严格调研已验证的优秀设计，再回来。没有出处不得发明方案，严禁猜测。
3. 选一条主用户路径，写完整现状链路（若是新模块则写「将接入的邻层」）。
...
5. 写清不引入什么、与旧代码的关系（替换哪一块并清理旧路径，而不是双轨、兼容层或防御性加固）。若分阶段落地，每一阶段都是干净切片：该阶段结束时旧路径已删，不得把半成品写进目标分层。
