...
3. 对照 [architecture.md](../instructions/rules/common/architecture.md)：**旧架构不打补丁**、**分阶段整理到干净**、**严禁残留技术债务**、内聚、模型分家、硬编码、**严禁兼容层**、**禁止防御性修补**、越级、下层不持有上层、**文件单一职责**、**API 不含实现**。没有出处与链路证据不得下结论，严禁猜测。
4. 打开范围内每个公开门面（`api/`、`YoXxx`、包入口、IPC 命令层）：只应看到类型与转发。出现算法、绘制、手势或 `import …internal` 记为越界。
5. 发现兼容性代码、兼容层，或拟议改动是在旧架构上打补丁/加固：先调研优秀设计，沿其全部相关数据链路查根因，重设计完整通路并清理旧代码，不得只删包装。只改相关链路上证明有问题的层；混职责文件按层拆开；范围内不留双轨、兼容代码、「非阻塞」半成品或任何技术债务。
6. 实现后再走同一条路径，留下通路证据。
...
声明范围（本阶段要整理到干净的切片）
  → 并行只读审查（每范围一份结论）
  → 处理该范围内全部未通过项（含原可被写成观察的分层债）
  → 全部修复落地
  → 对全部范围再开一轮只读复审
  → 同一波次全部通过（CLEAN、无未修分层问题）才收口
```

- 通过报告里不得附带未修的分层问题或技术债务，不得设「非阻塞观察」「已知债务」跳过修复环。范围内问题未清零即为 HAS_ISSUES。
- 优先级只排动手顺序，不决定能否跳过。
- 不要因为单范围变绿就再开一轮，也不要自行缩范围冒充完成。
- 下一阶段另声明范围；本阶段相关链路必须已是干净切片，禁止新旧双轨并存当「过渡」。
...
4. 问题清单：P0 / P1 / P2 + 文件:行 + 正确层；无则写无。级别只表示本阶段动手顺序，列出即须在本阶段闭合，不得标非阻塞。
5. 已通过项（含对照已声明的修复）
